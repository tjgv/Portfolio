...
export default function CaseStudySlideUpModal({
  ...
  if (isLightbox) {
    ...
    return createPortal(
      <div
        ref={overlayRef}
        className={`np1-popup-overlay np1-popup-overlay--lightbox np1-popup-overlay--${phase}`}
        role="dialog"
        aria-modal="true"
      ...
      >
        <button
          type="button"
          className="np1-popup-overlay__scrim"
          onClick={onClose}
          aria-label="Close overlay"
          tabIndex={-1}
        />

        <div className={`np1-lightbox${lightboxPanning ? ' np1-lightbox--panning' : ''}`}>
          <div
            className="np1-lightbox__frame"
          ...
          {lightboxShowPanHint ? (
            <div className="np1-lightbox__hint" role="status">
              <span className="np1-lightbox__hint-text">Hold space + Click to pan</span>
              <button
                type="button"
                className="np1-lightbox__hint-close"
          ...
          {hasMultiple ? (
            <>
              <button
                type="button"
                className="np1-lightbox__nav np1-lightbox__nav--prev"
              ...
              <button
                type="button"
                className="np1-lightbox__nav np1-lightbox__nav--next"
        ...
        <div className="np1-popup-overlay__close-host">
          <div
            className={`np1-modal-close-aap np1-popup-aap${closeReady ? ' np1-modal-close-aap--ready' : ''}`}
            style={{ ...closeGrowVars, ...closeReveal.style }}
          >
            <div
              className={`np1-popup-aap__cluster${closeProgress >= MODAL_CLOSE_REVEAL_START - 0.001 ? ' np1-popup-aap__cluster--reveal' : ''}`}
            >
              <div
                className="np1-grow-orb"
              ...
              >
                ...
                <div className="np1-grow-orb__seed np1-popup-aap__seed" aria-hidden />
              </div>

              <span
                className={`np1-modal-close-aap__icon-grow${closeReveal.iconVisible ? ' np1-modal-close-aap__icon-grow--visible' : ''}`}
                style={{
                  transform: `translate(-50%, calc(-50% + var(--grow-seed-y, 0))) scale(${closeReveal.iconScale})`,
                }}
                aria-hidden={!closeReveal.iconVisible}
              >
                <div className="np1-modal-close-aap__shell">
                  <button
                    type="button"
                    className="np1-modal-close-aap__action"
                  ...
                  >
                    <svg
                      className="np1-modal-close-aap__icon"
  ...
  return createPortal(
    <div
      ref={overlayRef}
      className={`np1-popup-overlay np1-popup-overlay--${phase}${variant === 'compact' ? ' np1-popup-overlay--compact' : ''}${variant === 'short' ? ' np1-popup-overlay--short' : ''}${hideHero ? ' np1-popup-overlay--no-hero' : ''}`}
      role="dialog"
      aria-modal="true"
      aria-label={ariaLabel}
      data-modal-theme="dark"
    >
      ...
      <div ref={scrollRef} className="np1-popup-overlay__scroll">
        <div className="np1-popup-overlay__stage">
          <div className="np1-popup-overlay__sheet" data-modal-content-wrapper="">
          {!hideHero ? (
            <div className="np1-popup-overlay__hero">
              <div className="np1-popup-overlay__hero-media">
                <img
                  className="np1-popup-overlay__hero-image"
                  src={heroImage}
                  alt={heroImageAlt ?? ''}
                />
              </div>
              <div className="np1-popup-overlay__hero-copy">
                <h2 className="np1-popup-overlay__hero-headline">{headline}</h2>
                {subhead ? (
                  <p className="np1-popup-overlay__hero-subhead">{subhead}</p>
                ) : null}
              </div>
            </div>
          ) : null}

          <div className="np1-popup-overlay__subsections" id="np1-modal-subsections">
            {quotes.map((quote) => (
              <section
                key={quote.id}
                className="np1-popup-overlay__subsection"
                aria-labelledby={`np1-modal-quote-${quote.id}`}
              >
                <blockquote className="np1-popup-overlay__quote">
                  <p id={`np1-modal-quote-${quote.id}`} className="np1-popup-overlay__quote-text">
                    &ldquo;{quote.text}&rdquo;
                    {quote.attribution ? (
                      <span className="np1-popup-overlay__quote-attribution">
                        {' '}
            ...
            {subsections.map((section) => {
              const sectionLabelId = `np1-modal-section-${section.id}`
              const sectionLabel =
              ...
              <section
                key={section.id}
                className={`np1-popup-overlay__subsection${section.carousel && !section.headline && !section.body ? ' np1-popup-overlay__subsection--carousel-only' : ''}`}
                ...
                {section.compare ? (
                  ...
                  <>
                    {section.headline || section.body ? (
                      ...
                      <h3 id={sectionLabelId} className="np1-popup-overlay__sr-only">
                        {sectionLabel}
                      </h3>
                    )}
                    {section.carousel ? (
                      <div className="np1-modal-carousel">
                        <ImageCarousel
                      ...
                      <div className="np1-popup-overlay__subsection-media">
                        <img
                          className="np1-popup-overlay__subsection-image"
                          src={section.image}
