  ...
  return (
    ...
    >
      <div className="np1-section__inner np1-results__inner">
        ...
        <div className="np1-results__phases">
          <div
            className="np1-results__stroke"
            aria-hidden="true"
            style={{ transform: `scaleX(${strokeProgress})` }}
          />

          <ul className="np1-results__list">
            {PHASES.map((phase, i) => {
              ...
              return (
                <li
                  key={phase.id}
                  className={`np1-results__item${isVisible ? ' np1-results__item--visible' : ''}`}
                >
                  <div
                    className={[
                      'np1-phase-circle',
                      `np1-phase-circle--${phase.tone}`,
                      `np1-phase-circle--${phase.borderStyle}`,
                      isActivated && 'np1-phase-circle--activated',
                    ]
                      .filter(Boolean)
                      .join(' ')}
                  >
                    <p className="np1-phase-circle__label">{phase.label}</p>
                    <p className="np1-phase-circle__title">{phase.title}</p>
                    <p
                      className={`np1-phase-circle__status${isActivated ? ' np1-phase-circle__status--visible' : ''}`}
                    >
                      {phase.status}
                    </p>
                  </div>
