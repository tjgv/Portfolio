...
export default function PopupRevealControl({
  ...
  if (usesSlideUpModal && modal) {
    ...
    overlayPortal = createPortal(
      <div
        className={`np1-popup-overlay np1-popup-overlay--${legacyOverlayPhase}`}
        role="dialog"
        aria-modal="true"
      ...
      >
        <button
          type="button"
          className="np1-popup-overlay__backdrop"
          onClick={closeOverlay}
          aria-label="Close overlay"
          tabIndex={-1}
        />
        <div className="np1-popup-overlay__content">
          <header className="np1-popup-overlay__header">
            <h3 className="np1-popup-overlay__headline">{LEGACY_OVERLAY_HEADLINE}</h3>
            <p className="np1-popup-overlay__subhead">{LEGACY_OVERLAY_SUBHEAD}</p>
          </header>
          <div className="np1-popup-overlay__panel">
            <img
              className="np1-popup-overlay__image"
        ...
        <button
          type="button"
          className="np1-popup-aap__action np1-popup-overlay__close"
          onClick={closeOverlay}
          aria-label="Close overlay"
        >
  ...
  const controlEl = (
    <div
      className={`np1-popup-reveal${visible ? ' np1-popup-reveal--visible' : ''}${pinned ? ' np1-popup-reveal--pinned' : ''} np1-popup-aap${expandActive ? ' np1-popup-aap--grow-done' : ''}${controlsReady ? ' np1-popup-aap--ready np1-popup-aap--revealed' : ''}${overlayOpen ? ' np1-popup-reveal--overlay-open' : ''}`}
      style={controlStyle}
      aria-hidden={!visible}
    >
      <div className="np1-popup-aap__stage">
        <div
          className="np1-popup-aap__morph-pill np1-popup-aap__morph-pill--measure"
          ref={pillMeasureRef}
          aria-hidden
        >
          <span className="np1-popup-aap__label" style={{ opacity: 1 }}>
            {ctaLabel}
          </span>
          <button type="button" className="np1-popup-aap__action" tabIndex={-1} style={{ opacity: 1 }}>
            {actionIcon}
          </button>
        </div>

        <div className={`np1-popup-aap__cluster${revealActive ? ' np1-popup-aap__cluster--reveal' : ''}`}>
          <div
            className="np1-popup-aap__play-orb"
          ...
          <div
            className={`np1-popup-aap__morph-pill${expandActive ? ' np1-popup-aap__morph-pill--expanding' : ''}`}
            aria-hidden={!revealActive}
          >
            <span className="np1-popup-aap__label">{ctaLabel}</span>
            <button
              type="button"
              className="np1-popup-aap__action"
              onClick={openOverlay}
  ...
  const pinnedPortal =
    pinned && visible
      ? createPortal(
          <div className="np1-popup-reveal-portal-host">{controlEl}</div>,
          document.body
        )
      : null
