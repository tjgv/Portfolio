...
function SideShotFigure({
  ...
  return (
    <figure className="np1c-side-shot__figure">
      {media ? (
        <div className="np1c-side-shot__media">{media}</div>
      ) : (
        <img className="np1c-side-shot__image" src={imageSrc} alt={imageAlt} loading="lazy" />
      )}
    </figure>
  )
}
...
function SideShotCopy({
  ...
  const bodyClassName =
    bodyVariant === 'body-1'
      ? 'np1c-side-shot__body np1c-side-shot__body--body-1'
      : 'np1c-side-shot__body'

  return (
    <div className={`np1c-side-shot__copy${stacked ? ' np1c-side-shot__copy--stacked' : ''}`}>
      {label ? <p className="np1c-side-shot__label">{label}</p> : null}
      <h2 className="np1c-side-shot__title">{title}</h2>
...
export default function SideShotSection({
  ...
  const sectionStyle: CSSProperties | undefined =
    imageScale !== 1 ? ({ '--np1c-side-shot-image-scale': imageScale } as CSSProperties) : undefined

  return (
    <section
      ref={sectionRef}
      className={`np1c-section np1c-side-shot np1c-side-shot--${isStacked ? 'stacked' : `image-${imagePosition}`}`}
      data-dev-section={devSection}
      aria-label={ariaLabel}
      style={sectionStyle}
    >
      {isStacked ? (
        <div className="np1c-side-shot__inner">
          <div className="np1c-side-shot__stack">
            <SideShotCopy label={label} title={title} stacked bodyVariant={bodyVariant}>
              {children}
            </SideShotCopy>
            <div className="np1c-side-shot__media-block">
        ...
        <div className="np1c-side-shot__grid">
          <div className="np1c-side-shot__copy">
            {label ? <p className="np1c-side-shot__label">{label}</p> : null}
            <h2 className="np1c-side-shot__title">{title}</h2>
            <div className={bodyClassName}>{children}</div>
            {pillReveal}
