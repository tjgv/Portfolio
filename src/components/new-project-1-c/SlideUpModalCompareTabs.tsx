...
export default function SlideUpModalCompareTabs({
  ...
  const panelId = `np1c-modal-compare-${sectionId}`

  return (
    <div className="np1c-modal-compare">
      <ModalSectionTextStack sectionId={sectionId} headline={headline} body={body} />

      <div
        className="np1c-modal-compare__tabs"
        role="tablist"
      ...
      >
        <button
          ...
          className={`np1c-modal-compare__tab${activeTab === 'before' ? ' np1c-modal-compare__tab--active' : ''}`}
        ...
        <button
          ...
          className={`np1c-modal-compare__tab${activeTab === 'after' ? ' np1c-modal-compare__tab--active' : ''}`}
      ...
      <div
        ...
        className={`np1c-modal-compare__panel${activeTab === 'after' ? ' np1c-modal-compare__panel--after' : ''}`}
      >
        <SlideUpModalLoopVideo
          key={`${sectionId}-${activeTab}-${activeVideo.src}`}
          src={activeVideo.src}
          ariaLabel={activeVideo.ariaLabel}
          className="np1c-modal-compare__video"
          videoRef={videoRef}
      ...
      <div className="np1c-modal-compare__video-controls">
        <button
          type="button"
          className="np1c-modal-video-btn"
        ...
        <button
          type="button"
          className="np1c-modal-video-btn np1c-modal-video-btn--progress"
          onClick={handlePlayPause}
          aria-label={isPlaying ? 'Pause video' : 'Play video'}
        >
          <svg className="np1c-modal-video-progress" viewBox="0 0 48 48" aria-hidden>
            <circle
              className="np1c-modal-video-progress__track"
              cx="24" cy="24" r={PROGRESS_RADIUS}
            />
            <circle
              className="np1c-modal-video-progress__fill"
              cx="24" cy="24" r={PROGRESS_RADIUS}
              strokeDasharray={PROGRESS_CIRCUMFERENCE}
              strokeDashoffset={strokeOffset}
            />
          </svg>
          <span className="np1c-modal-video-btn__icon">
