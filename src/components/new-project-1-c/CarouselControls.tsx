...
export default function CarouselControls({
  ...
  return (
    <>
      <div
        className={`np1c-carousel-edge-arrows${ready ? ' np1c-carousel-edge-arrows--visible' : ''}`}
        style={{ '--aap-ui-opacity': uiOpacity } as CSSProperties}
      >
        <button
          type="button"
          className="np1c-carousel-edge-arrow np1c-carousel-edge-arrow--prev"
          onClick={goPrev}
        ...
        <button
          type="button"
          className="np1c-carousel-edge-arrow np1c-carousel-edge-arrow--next"
          onClick={goNext}
      ...
      <div
        className={[
          'np1c-carousel-controls',
          'np1c-carousel-aap',
          ready ? 'np1c-carousel-aap--ready np1c-carousel-aap--revealed' : '',
          isManual ? 'np1c-carousel-aap--manual' : '',
        ]
          .filter(Boolean)
          .join(' ')}
        style={manualStyle}
      >
        <div className="np1c-carousel-aap__stage">
          <div className="np1c-carousel-aap__cluster">
            <div className="np1c-carousel-aap__nav-shell" role="tablist" aria-label="Carousel slides">
              <div className="np1c-carousel-aap__nav">
                {slides.map((slide, index) => {
                  ...
                  return (
                    <div
                      key={slide.id}
                      role="presentation"
                      className={`np1c-carousel-aap__dot-track${isActive ? ' np1c-carousel-aap__dot-track--active' : ''}`}
                    >
                      <button
                        type="button"
                        role="tab"
                        aria-selected={isActive}
                        aria-controls={`np1c-carousel-slide-${slide.id}`}
                        className={`np1c-carousel-aap__dot${isActive ? ' np1c-carousel-aap__dot--active' : ''}`}
                        onClick={() => {
                          onSelectSlide(index)
                          onPauseAutoplay?.()
                        }}
                      ...
                      >
                        ...
                        {!isManual ? (
                          <span
                            className="np1c-carousel-aap__dot-progress"
                            style={{
            ...
            {!isManual ? (
              <div className="np1c-carousel-aap__play-orb">
                ...
                <button
                  type="button"
                  className="np1c-grow-orb__seed np1c-carousel-aap__play-shell"
                  onClick={onPlayPause}
                  disabled={!ready}
                  aria-label={
                ...
                >
                  <span className="np1c-carousel-aap__play-icon" aria-hidden>
                    {ended ? (
