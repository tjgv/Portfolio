...
function getAnchorClasses(x: number, y: number): string {
  ...
  if (x >= 72) classes.push('np1c-info-bubble-anchor--right')
  else if (x <= 28) classes.push('np1c-info-bubble-anchor--left')
  else classes.push('np1c-info-bubble-anchor--center-x')

  if (y >= 78) classes.push('np1c-info-bubble-anchor--bottom')
  else classes.push('np1c-info-bubble-anchor--top')
...
function InfoBubble({
  ...
  return (
    <div
      className={`np1c-info-bubble-anchor ${getAnchorClasses(hotspot.x, hotspot.y)}${entered ? ' np1c-info-bubble-anchor--entered' : ''}`}
      style={{
        left: `${hotspot.x}%`,
        top: `${hotspot.y}%`,
    ...
    >
      <button
        type="button"
        className={`np1c-info-bubble${expanded ? ' np1c-info-bubble--expanded' : ''}`}
      ...
      >
        <span className="np1c-info-bubble__icon" aria-hidden>
          i
        </span>
        <span className="np1c-info-bubble__body" id={titleId}>
          <span className="np1c-info-bubble__body-inner">
            <span className="np1c-info-bubble__title">{hotspot.title}</span>
            <span className="np1c-info-bubble__text">{hotspot.body}</span>
          </span>
        </span>
      </button>
    </div>
...
export default function CurrentToolHotspotMap({ src, alt, hotspots }: CurrentToolHotspotMapProps) {
  ...
  return (
    <div ref={mapRef} className="np1c-hotspot-map" onClick={closeAll}>
      <div className="np1c-hotspot-map__stage" style={{ aspectRatio: IMAGE_ASPECT }}>
        <ImgWithLoader className="np1c-hotspot-map__image" src={src} alt={alt} />
        <div className="np1c-hotspot-map__layer" aria-live="polite">
