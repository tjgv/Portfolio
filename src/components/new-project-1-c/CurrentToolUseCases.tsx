...
export default function CurrentToolUseCases({ items }: CurrentToolUseCasesProps) {
  ...
  return (
    <div className="np1c-use-cases-carousel">
      <div
        ref={galleryRef}
        className="np1c-use-cases"
        role="region"
        aria-label="Experience use cases"
        tabIndex={0}
      >
        <ul ref={trackRef} className="np1c-use-cases__track">
          {items.map((item, index) => {
            ...
            return (
              <li
                ...
                className={[
                  'np1c-use-case-card',
                  isActive && 'np1c-use-case-card--active',
                ].filter(Boolean).join(' ')}
                aria-hidden={!isActive}
              >
                <div className="np1c-use-case-card__media">
                  <ImgWithLoader src={item.image} alt="" aria-hidden onLoad={updatePadding} />
                </div>
                <p className="np1c-use-case-card__text">{item.body}</p>
              </li>
            )
          })}
        </ul>
      ...
      <div className="np1c-use-cases-carousel__controls">
        <nav className="np1c-quote-card__nav" aria-label="Use case slide navigation">
          <button
            type="button"
