...
const EMBED_SECTIONS: readonly EmbedSection[] = [
  ...
  },
  {
    ...
    extra: (
      <ResultsPhasesAnimation className="np1c-embed-results-phases" capStrokeAtLastCircle />
    ),
  },
] as const
  ...
  return (
    ...
    >
      ...
      <main className={`project-main${embedded ? ' project-main--embedded' : ''}`}>
        {embedded ? (
          <div className="np1c-embed-banner-block" data-dev-section="hero">
            <img
              className="np1c-embed-banner-block__img"
        ...
        <div className="np1c-content" data-dev-section="content">
              ...
              <div className="np1c-split">
                ...
                >
                  {GLANCE_BLOCKS.map((block, index) => (
                    <article
                      key={block.id}
                      className="np1c-glance-block"
                      style={{ '--np1c-glance-stagger': index } as CSSProperties}
                    >
                      <div className="np1c-glance-block__head">
                        <span className="np1c-glance-block__icon" aria-hidden>
                          {block.icon}
                        </span>
                        <h3 className="np1c-glance-block__label">{block.label}</h3>
                      </div>
                      <div className="np1c-glance-block__content">
                        {block.value ? (
                          <p className="np1c-glance-block__value">{block.value}</p>
                        ) : null}
                        {block.pairs ? (
                          <div className="np1c-glance-block__pairs">
                            {block.pairs.map((pair) => (
                              <div key={pair.id} className="np1c-glance-block__pair">
                                <span className="np1c-glance-block__pair-primary">{pair.primary}</span>
                                <span className="np1c-glance-block__pair-secondary">{pair.secondary}</span>
                              </div>
                            ))}
                          </div>
                        ) : null}
                        {block.paragraph ? (
                          <p className="np1c-glance-block__paragraph">{block.paragraph}</p>
                        ) : null}
                      </div>
                    </article>
                  ))}
          ...
          <section className="np1c-section np1c-intro" data-dev-section="intro" aria-label="Introduction">
            <div className="np1c-section__inner np1c-intro__inner">
              <div className="np1c-embed-block__text np1c-embed-block__text--lead">
                <div className="np1c-split">
                  <h2 className="np1c-split__headline">
                    Preparing an internal tool for an external release.
                  </h2>
                  <p className="np1c-split__body">
        ...
        {embedded && (
          <div className="np1c-embed-sections">
            {EMBED_SECTIONS.map((section) => (
              <div
                key={section.id}
                className="np1c-embed-block"
                data-dev-section={section.id}
              >
                <div className="np1c-embed-block__text">
                  <h2 className="np1c-embed-header">{section.header}</h2>
                  <div className="np1c-embed-block__body">{section.body}</div>
                </div>
                {section.images?.length === 2 ? (
                  <div className="np1c-embed-two-images">
                    {section.images.map((image) => (
                      <ImgWithLoader key={image.src} src={image.src} alt={image.alt} />
                    ))}
                  </div>
                ) : section.images?.length === 1 ? (
                  <div className="np1c-embed-full-width">
                    <ImgWithLoader src={section.images[0].src} alt={section.images[0].alt} />
                  </div>
                ) : null}
                {section.videos?.length ? (
                  <div className="np1c-embed-video-stack">
                    {section.videos.map((video) => (
                      <div key={video.src} className="np1c-embed-full-width">
                        <div
                          className="np1c-embed-video-frame"
            ...
            return (
              ...
              >
                <div
                  ...
                  className={`np1c-section__inner${isGoal ? ' np1c-stub__reveal' : ''}`}
                >
                  <p className="np1c-stub__label">{section.label}</p>
                  <h2 className="np1c-stub__title">{section.title}</h2>
                  {'body' in section && section.body ? (
                    <p className="np1c-stub__body">{section.body}</p>
                  ) : null}
                </div>
              </section>
            )
        ...
        {!embedded && (
          <footer className="np1c-footer" data-dev-section="footer">
            <div className="np1c-footer__inner">
            ...
            <p className="np1c-footer__quote">
